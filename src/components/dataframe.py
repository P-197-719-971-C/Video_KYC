...
def dataframe(cap , smoothing_frames:int = 3, order: int = 20):
    ...
    idList = [159, 23, 130, 243, 386, 374, 382, 263, 10, 152, 323, 93, 0, 2, 17, 291, 61, 205, 425]
    # nose tip = [0]
    ...
    while cap.isOpened():
        ...
        if results.multi_face_landmarks:
            ...
            ratio_eye = calculate_eye_aspect_ratio(face, img)
            intersection_y, ratio_face = facial_htw_ratio(face, img)
            ratio_smile = smile_ratio(face, img)
            
            if intersection_y < face[0].y:
                ratioList.append(ratio_eye)
